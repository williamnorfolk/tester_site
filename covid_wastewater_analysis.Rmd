...
pcdc1 <- plotly::plot_ly() %>%
  plotly::add_lines(x = ~date_vec_both, y = ~both_trend,
                    ...
                    line = list(color = '#1B9E77', size = 8, opacity = 0.65),
                    showlegend = FALSE) %>%
              layout(yaxis = list(title = "Total Log10 SARS CoV-2 Copies in Wastewater", 
                                 showline = TRUE,
                                 automargin = TRUE,
                                 color = '#1B9E77')) %>%
                layout(xaxis = list(title = "Date"))  %>%
  
  plotly::add_lines(x = ~date, y = ~X7_day_ave_clarke,
                    data = only_background,
                    yaxis = "y2",
                    color = '#D95F02') %>%
  layout(yaxis2 = list(title = "7-Day Moving Average Cases", 
                       overlaying = "y", 
                       side = "right",
                       automargin = TRUE,
                       showline = TRUE,
...
pcdc2 <- plotly::plot_ly() %>%
  plotly::add_lines(x = ~date, y = ~sum_copies_both,
                    data = rejoin,
                    hoverinfo = "text",
                    #text = ~paste('</br> Date: ', date,
                    #              '</br> Median Log Copies: ', round(both_trend, digits = 2)),
                    line = list(color = '#1B9E77', size = 8, opacity = 0.65),
                    showlegend = FALSE) %>%
              layout(yaxis = list(title = "Total SARS CoV-2 Copies in Wastewater", 
                                 showline = TRUE,
                                 automargin = TRUE,
                                 color = '#1B9E77')) %>%
                layout(xaxis = list(title = "Date"))  %>%
  ...
  plotly::add_lines(x = ~date, y = ~X7_day_ave_clarke,
                    ...
                    yaxis = "y2",
                    color = '#D95F02') %>%
  layout(yaxis2 = list(title = "7-Day Moving Average Reported Cases", 
                       overlaying = "y", 
                       side = "right",
                       automargin = TRUE,
                       showline = TRUE,
...
pcdc3 <- plotly::plot_ly() %>%
  plotly::add_lines(x = ~date, y = ~log_sum_copies_both,
                    data = rejoin,
                    hoverinfo = "text",
                   # text = ~paste('</br> Date: ', date_vec_both,
                  #                '</br> Median Log Copies: ', round(both_trend, digits = 2)),

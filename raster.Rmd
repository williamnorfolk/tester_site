...
lines <- rejoin %>% ggplot() +
  ...
  annotate("pointrange", x = 13.41667, y = 1.5, ymin = 0.95, ymax = 2.05, color = "red", size =1) +
  theme_classic() +
  xlab("Log Viral Load")
...
lines <- lines +   
  ...
  annotate("text", x = 12.3, y = 0.95, label = "Lowest Observed") +
  annotate("text", x = 14.2, y = 0.95, label = "Highest Observed") +
  labs(caption = "This Week's Viral Load 2.61e+13 Copies")
...
perc.rank(rejoin$log_sum_copies_both, 13.41667)
```
...
add_perc_rank <- try %>% mutate(percrank=rank(log_sum_copies_both)/length(log_sum_copies_both))
```

31.14754

```{r}
write.csv(base_data, "./n1_and_n2_cleaned_cases.csv")
```

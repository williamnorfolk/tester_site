...
sum_data <- sum_data %>% 
  ...
  summarize(mean_copy_num_L = mean(mean_copy_num_L)) %>%
  ungroup()
...
sum_data <- sum_data %>% select(-c(Facility)) %>%
  ...
  summarize(mean_copy_num_L = sum(mean_copy_num_L)) %>%
  ungroup() %>%
  mutate(log_copy_per_L = log10(mean_copy_num_L))
...
remove_hits_front <- subset(remove_hits, date >= as.Date("2020-06-10"))
...
seven_day_cumulative_front <- seven_day_cumulative_front[!seven_day_cumulative_front %in% 924 ]
seven_day_cumulative_front

#create a vector of the 7-day cumulative totals where sampling is the middle of the 7 days
remove_hits_mid <- subset(remove_hits, date >= as.Date("2020-06-13"))
...
remove_hits_back <- subset(remove_hits, date >= as.Date("2020-06-16"))
...
#n1
plot_data_n1 <- only_n1 %>% select(c(date, log_copy_per_L))
...
#n2
plot_data_n2 <- only_n2 %>% select(c(date, log_copy_per_L))
...
n1_front <- plot_data_n1 %>% ggplot() + 
  geom_point(aes(x = log_copy_per_L, y = seven_day_front, color = "Before Sample")) + 
  geom_smooth(aes(x = log_copy_per_L, y = seven_day_front), method = "lm", se = FALSE) +
  stat_smooth_func(aes(x = log_copy_per_L, y = seven_day_front), 
                   geom ="text", method = "lm", hjust =0, vjust = -1, parse = TRUE) +
  theme_classic() +
  xlab("Mean Log SARS CoV-2 Copies/L") +
...
n1_mid <- plot_data_n1 %>% ggplot() + 
  geom_point(aes(x = log_copy_per_L, y = seven_day_mid, color = "Before Sample")) + 
  geom_smooth(aes(x = log_copy_per_L, y = seven_day_mid), method = "lm", se = FALSE) +
    stat_smooth_func(aes(x = log_copy_per_L, y = seven_day_mid), 
...
n1_back <- plot_data_n1 %>% ggplot() + 
  geom_point(aes(x = log_copy_per_L, y = seven_day_back, color = "Before Sample")) + 
  geom_smooth(aes(x = log_copy_per_L, y = seven_day_back), method = "lm", se = FALSE) +
    stat_smooth_func(aes(x = log_copy_per_L, y = seven_day_back), 
                   geom ="text", method = "lm", hjust =0, vjust = 1, parse = TRUE) +
  theme_classic() +
  xlab("Mean Log SARS CoV-2 Copies/L") +
  ylab("7-Day Cumulative Cases")+

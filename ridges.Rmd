...
```{r}
combined_targets <- full_join(only_n1, only_n2)
  
combined_targets_ave <- combined_targets %>% select(-c(Facility, cases_cum_clarke, new_cases_clarke, cases_per_100000_clarke, collection_num, mean_copy_num_uL_rxn, sd_L, sd_total_copies)) %>%
  group_by(date, X7_day_ave_clarke, target) %>%
  summarize_if(is.numeric, sum) %>%
  ungroup()
```

```{r}
ridge_plot <- combined_targets_ave %>% ggplot() +
  geom_density_ridges(aes(x = mean_total_copies, y = as.factor(X7_day_ave_clarke), fill = as.factor(X7_day_ave_clarke)),scale = 3)

ridge_plot + theme(legend.position = "none")
```

```{r}
combined_targets <- combined_targets %>% mutate(log_mean_total_copies = log10(mean_total_copies))

gg <- combined_targets %>% ggplot() +
  geom_density_ridges(aes(x = log_mean_total_copies, y = Facility, fill = Facility),scale = 3)
...
```{r}
only_7_day <- combined_targets_ave %>% select(c(date, X7_day_ave_clarke))

add_ave <- full_join(rejoin, only_7_day)
```

```{r}
good_ridge_plot <- add_ave %>% ggplot() +
  geom_density_ridges(aes(x = sum_copies_both, y = as.factor(X7_day_ave_clarke), fill = as.factor(X7_day_ave_clarke)),scale = 3)

good_ridge_plot + theme(legend.position = "none")
```

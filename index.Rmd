...
Detection trends in SARS CoV 2 markers in Athens sewage were split this week. During the collection period (Oct 11 – 12), the concentrations of SARS CoV 2 markers at WRF B declined from last week, to at or below the limit of detection. However, levels at WRF A increased to >500,000 copies per L, after declining to near the limit of detection the prior week. Given that WRF B levels were low, it is too early to know if the increased signal at WRF A is indicative of possible uptick in transmission or due to inherent variability in sample collection. If there is increased transmission, we would expect to see the trend in wastewater continue next week as well as an increase in reported cases.  
...
Levels of SARS-CoV-2 continues to decrease in wastewater samples collected from Athens-Clarke County. These results are consistent with the decrease in the number of newly-reported cases over the past two weeks.  
...
```{r out.height = "500%", out.width = "100%", echo=FALSE, message=FALSE, warning=FALSE}
load(file = "./p_wrf_b.rda")
load(file = "./date_vec_n1b.rda")
load(file = "./date_vec_n2b.rda")
